 ...
 * exist and belong to the authenticated user, and to resolve the full folder
 * tree (paths) for ZIP exports and analytics. The {@code X-User-Id} header is
...
public interface FolderServiceClient {
    ...
    /**
     * Retrieves every non-deleted folder owned by the user (flat list).
     *
     * @param ownerId the authenticated user's ID (forwarded as X-User-Id)
     * @return the standard response containing all folders
     */
    @GetMapping
    StandardResponse<List<FolderResponse>> getAllFolders(@RequestHeader("X-User-Id") Long ownerId);
}
